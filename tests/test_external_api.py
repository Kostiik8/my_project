...
from src.external_api import convert_to_rub, get_exchange_rate, read_json_transactions
...
def test_get_exchange_rate(mock_get):
    ...
    assert rate == expected_rate, f"Expected {expected_rate}, but got {rate}"
...
def test_convert_to_rub(mock_get_exchange_rate):
    ...
    assert amount_in_rub == expected_amount_in_rub, f"Ожидается {expected_amount_in_rub}, но дает {amount_in_rub}"
