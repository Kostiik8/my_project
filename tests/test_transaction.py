...
class TestTransactionsFunctions(unittest.TestCase):
    ...
    @patch("builtins.open", new_callable=mock_open, read_data="""id;status;timestamp;amount;currency;currency_code;from;to;description
...
4699552;EXECUTED;2022-03-23T08:29:37Z;23423;Peso;PHP;Discover 7269000803370165;American Express 1963030970727681;Перевод с карты на карту""")
    def test_get_transactions_csv(self, mock_open):
        ...
        with patch('sys.stdout', new=StringIO()) as fake_out:
            ...
            self.assertIn("['4137938', 'EXECUTED', '2023-01-04T13:13:34Z', '15560', 'Real', 'BRL', '', "
                          "'Счет 38164279390569873521', 'Открытие вклада']", fake_out.getvalue())
            self.assertIn("['4699552', 'EXECUTED', '2022-03-23T08:29:37Z', '23423', 'Peso', 'PHP', "
                          "'Discover 7269000803370165', 'American Express 1963030970727681',"
                          " 'Перевод с карты на карту']", fake_out.getvalue())
    ...
    def test_get_transactions_excel(self, mock_read_excel):
        data = {
            ...
            "timestamp": ["2023-01-04T13:13:34Z", "2022-03-23T08:29:37Z"],
            "amount": [15560, 23423],
            "currency": ["Real", "Peso"],
